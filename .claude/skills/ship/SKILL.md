...
- Propose which files to stage. **Never stage** these:
  - Secrets: `.env*`, `*.pem`, `*.key`, `credentials.json`, `appsettings.*.json` with secrets
  - Build output: `bin/`, `obj/`, `*.dll`, `*.exe`, `*.nupkg`
  - Generated: `*.g.cs`, `*.Designer.cs`, migrations auto-generated output
