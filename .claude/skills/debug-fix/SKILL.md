...
- Run related tests for regressions: `dotnet test --filter "FullyQualifiedName~<Namespace>"`.
- Run the build: `dotnet build`.
- Temporarily revert your fix and confirm the new test fails (proves the test catches the bug).

**--fast**:
- Run only tests directly relevant to the changed code: `dotnet test --filter "FullyQualifiedName~<Namespace>"`.
- Run the build: `dotnet build`.
