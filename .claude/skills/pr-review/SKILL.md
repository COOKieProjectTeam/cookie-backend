...
| `security-reviewer` | Auth, input handling, queries, tokens, session management, file path construction, SQL strings. |
| `performance-reviewer` | Endpoints, DB queries, EF Core, loops over collections, caching, async patterns. Skip for pure-docs, config-only diffs. |
| `doc-reviewer` | `.md` changes, significant XML doc comment changes, API docs. |

**Dispatch all applicable reviewers in PARALLEL.** Send one message that contains one `Task` tool call per applicable reviewer (use `subagent_type` matching the reviewer name). Do NOT invoke them sequentially.

If only one reviewer applies, a single `Task` call is fine.
